...
1. Copy script `vigor-stats.py` to a directory accessible by Telegraf
2. Make the script executable `chmod +x draytekstatus.py`
...
```
[[inputs.exec]]
  commands = ["python3 /opt/telegraf/vigor-stats.py"]
  timeout = "15s"
  data_format = "influx"
```
